  ...
  return (
    <Box className="dashboard_main">
      <Box className="header_main">
        <Typography className="page_title">Certificates Dashboard</Typography>
      </Box>
      ...
      <Grid container className="grid_header" sx={{ backgroundColor: "#4b1d77", color: "white", p: 2 }}>
        ...
        <Grid item size={2}> <Typography>Makers Approval</Typography> </Grid>
      ...
      {projectsData.length > 0 ? (
        ...
        <Grid container key={index} sx={{ p: 1, borderBottom: "1px solid #ddd", alignItems: "center" }} >
          ...
          <Grid item size={2}>  {item.makers_approval === "success" ? (  <CheckCircle sx={{ color: "green" }} /> ) : (
          <Typography color="warning.main">Pending</Typography>)}</Grid>
          <Grid item size={2}> {item.approver_status === "success" ? (<CheckCircle sx={{ color: "green" }} />  ) : (
